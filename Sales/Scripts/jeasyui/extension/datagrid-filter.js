(function($){
	...
	function myLoadFilter(data){
		...
		if (!opts.remoteFilter && opts.filterRules.length){
	...
	function init(target, filters){
		...
		function createFilter(frozen){
			...
			var tr = $('<tr class="datagrid-header-row datagrid-filter-row"></tr>').appendTo(table.find('tbody'));
		...
		function createFilterButton(container, operators){
			...
			var btn = $('<a class="datagrid-filter-btn">&nbsp;</a>').addClass(opts.filterBtnIconCls).appendTo(container);
