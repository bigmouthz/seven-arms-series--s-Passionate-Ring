...
//    if (debug) { console.log("Init:sourcePool"); console.log(sourcePool.length); console.log(sourcePool); }
...
function onccSelect(record) {
   ...
   isFilter = false;
    if (cc[0] == "X") {
        ...
        isFilter = false;
  //      if (debug) { console.log("Init:tmpPoolX"); console.log(tmpPool.length); console.log(tmpPool); }
    } else if (cc[0] == "Y") {
        tmpPool = _.filter(sourcePool, function (item) { return item.YFlag == "0";  });
        isFilter = false;
  //      if (debug) { console.log("Init:tmpPoolY"); console.log(tmpPool.length); console.log(tmpPool); }
    } else if (cc == "F0001") {
...
function change() {
    ...
    if (tmpPool.length > 0) {
        ...
        showresult.html("<font style='font-size:1.0em;line-height:70px;color:black;font-family:SimHei;'>" + tmpPool[i].Name + "</font>");
        showresultid.val(tmpPool[i].ID);
        curtmp = tmpPool[i];
    } else {
        showresult.html("<font style='font-size:1.0em;line-height:70px;color:black;font-family:SimHei;'></font>");
        showresultid.val("");
        curtmp = "";
    }
};
...
//得到下一轮抽奖池
function getnexttmppool(result, cc) {
    if (result == "") return;
    if (isFilter == true) {
        /*取下次过滤名单---Begin*/
        switch (cc) {
            case "F0001":
                {
                    for (var i = 0; i < bitPool.length; i++) {
                        if (bitPool[i].ID == result.ID) {
                            ...
                            tmpPool = _.filter(bitPool, function (item) { return item.Flag_0001 == "0"; });
                            //if (debug) { console.log("bitPool_0001"); console.log(tmpPool.length); console.log(tmpPool); }
                            localStorage['bitPool'] = JSON.stringify(bitPool);
                ...
                case "F0010":
                {
                    for (var i = 0; i < bitPool.length; i++) {
                        if (bitPool[i].ID == result.ID) {
                            ...
                            tmpPool = _.filter(bitPool, function (item) { return item.Flag_0010 == "0"; });
                            //if (debug) { console.log("bitPool_0010"); console.log(tmpPool.length); console.log(tmpPool); }
                            localStorage['bitPool'] = JSON.stringify(bitPool);
                            for (var j = 0; j < doublebitPool.length; j++) {
                                if (filter_doublefilterbitPool(result.ID, doublebitPool[j].ID)) {
                                    doublebitPool[j].Flag_0011 = "Relation" + result.ID;
            ...
            case "F0100":
                {
                    for (var i = 0; i < bitPool.length; i++) {
                        if (bitPool[i].ID == result.ID) {
                            ...
                            tmpPool = _.filter(bitPool, function (item) { return item.Flag_0100 == "0"; });
                            //if (debug) { console.log("bitPool_0100"); console.log(tmpPool.length); console.log(tmpPool); }
                            localStorage['bitPool'] = JSON.stringify(bitPool);
                            for (var j = 0; j < doublebitPool.length; j++) {
                                if (filter_doublefilterbitPool(result.ID, doublebitPool[j].ID)) {
                                    doublebitPool[j].Flag_0011 = "Relation" + result.ID;
            ...
            case "F1000":
                {
                    for (var i = 0; i < bitPool.length; i++) {
                        if (bitPool[i].ID == result.ID) {
                            ...
                            tmpPool = _.filter(bitPool, function (item) { return item.Flag_1000 == "0"; });
                            //if (debug) { console.log("bitPool_1000"); console.log(tmpPool.length); console.log(tmpPool); }
                            localStorage['bitPool'] = JSON.stringify(bitPool);
                            for (var j = 0; j < doublebitPool.length; j++) {
                                if (filter_doublefilterbitPool(result.ID, doublebitPool[j].ID)) {
                                    doublebitPool[j].Flag_0011 = "Relation" + result.ID;
                ...
                case "F0011":
                {
                    for (var i = 0; i < doublebitPool.length; i++) {
                        if (doublebitPool[i].ID == result.ID) {
                            ...
                            tmpPool = _.filter(doublebitPool, function (item) { return item.Flag_0011 == "0"; });
                            //if (debug) { console.log("doublebitPool_0011"); console.log(tmpPool.length); console.log(tmpPool); }
                            localStorage['doublebitPool'] = JSON.stringify(doublebitPool);
                            break;
                        }
                    }
            ...
            case "F0101":
                {
                    for (var i = 0; i < doublebitPool.length; i++) {
                        if (doublebitPool[i].ID == result.ID) {
                            ...
                            tmpPool = _.filter(doublebitPool, function (item) { return item.Flag_0101 == "0"; });
                            //if (debug) { console.log("doublebitPool_0101"); console.log(tmpPool.length); console.log(tmpPool); }
                            localStorage['doublebitPool'] = JSON.stringify(doublebitPool);
                            break;
                        }
                    }
            ...
            case "F0110":
                {
                    for (var i = 0; i < doublebitPool.length; i++) {
                        if (doublebitPool[i].ID == result.ID) {
                            ...
                            tmpPool = _.filter(doublebitPool, function (item) { return item.Flag_0110 == "0"; });
                            //if (debug) { console.log("doublebitPool_0110"); console.log(tmpPool.length); console.log(tmpPool); }
                            localStorage['doublebitPool'] = JSON.stringify(doublebitPool);
                            break;
                        }
                    }
            ...
            case "F1001":
                {
                    for (var i = 0; i < doublebitPool.length; i++) {
                        if (doublebitPool[i].ID == result.ID) {
                            ...
                            tmpPool = _.filter(doublebitPool, function (item) { return item.Flag_1001 == "0"; });
                            //if (debug) { console.log("doublebitPool_1001"); console.log(tmpPool.length); console.log(tmpPool); }
                            localStorage['doublebitPool'] = JSON.stringify(doublebitPool);
                            break;
                        }
                    }
            ...
            case "F1010":
                {
                    for (var i = 0; i < doublebitPool.length; i++) {
                        if (doublebitPool[i].ID == result.ID) {
                            ...
                            tmpPool = _.filter(doublebitPool, function (item) { return item.Flag_1010 == "0"; });
                            //if (debug) { console.log("doublebitPool_1010"); console.log(tmpPool.length); console.log(tmpPool); }
                            localStorage['doublebitPool'] = JSON.stringify(doublebitPool);
                            break;
                        }
                    }
            ...
            case "F1100":
                {
                    for (var i = 0; i < doublebitPool.length; i++) {
                        if (doublebitPool[i].ID == result.ID) {
                            ...
                            tmpPool = _.filter(doublebitPool, function (item) { return item.Flag_1100 == "0"; });
                            //if (debug) { console.log("doublebitPool_1100"); console.log(tmpPool.length); console.log(tmpPool); }
                            localStorage['doublebitPool'] = JSON.stringify(doublebitPool);
                            break;
                        }
                    }
        ...
        for (var i = 0; i < sourcePool.length; i++) {
            ...
            if (sourcePool[i].ID == result.ID) {
                if (cc[0] == "X") {
                    ...
                    //if (debug) { console.log("tmpPoolX"); console.log(tmpPool.length); console.log(tmpPool); }
                } else if (cc[0] == "Y") {
                    sourcePool[i].YFlag = cc;
                    tmpPool = _.filter(sourcePool, function (item) { return item.YFlag == "0"; });

                    //if (debug) { console.log("tmpPoolY"); console.log(tmpPool.length); console.log(tmpPool); }
                } else {
                    sourcePool[i].Flag = cc;
                    tmpPool = _.filter(sourcePool, function (item) { return item.Flag == "0"; });

                    //if (debug) { console.log("tmpPool"); console.log(tmpPool.length); console.log(tmpPool); }
                }
                //if (debug) { console.log("sourcePool"); console.log(sourcePool.length); console.log(sourcePool); }
                //localStorage['sourcePool'] = JSON.stringify(sourcePool);
                break;
...
function viewF(rsx) {
    ...
    for (var x = 0; x < rsx.length; x++) {
        ...
        || cc == "F0011" || cc == "F0110" || cc == "F1001" || cc == "F1010" || cc == "F1100" || cc == "F0101") {
         tmp = rsx[x]; 
         rs.push("<font style='font-size:6.5em;line-height:70px;color:black;font-family:SimHei;'>" + tmp.Name + "</font>");
         rs.push("<br>");
        }
    }
    //if (debug) { console.log("rs"); console.log(rs.length); console.log(rs); }
...
function viewOXY(rsx) {
    ...
    for (var x = 0; x < rsx.length; x++) {
        ...
        if (tmp) {
             if (count <= 10) {
                rs.push("<font style='font-size:0.85em;line-height:50px;color:black;font-family:SimHei;'>" + tmp.Name + "</font>");
                rs.push("<br>");                
            } else if (count <= 20) {
                rs.push("<font style='font-size:1.0em;line-height:70px;color:black;font-family:SimHei;'>" + tmp.Name + "</font>");
                if (x % 5 == 0) {
                    rs.push("&nbsp;&nbsp;");
                } else if (x % 5 == 1) {
                    rs.push("&nbsp;&nbsp;");
                ...
                rs.push("<font style='font-size:0.45em;line-height:5px;color:black;font-family:SimHei;'>" + tmp.Name + "</font>");
                if (x % 6 == 0) {
                    rs.push("&nbsp;&nbsp;");
                } else if (x % 6 == 1) {
                    rs.push("&nbsp;&nbsp;");
                ...
                rs.push("<font style='font-size:0.40em;line-height:5px;color:black;font-family:SimHei;'>" + tmp.Name + "</font>");
                if (x % 10 == 0) {
                    rs.push("&nbsp;&nbsp;");
                } else if (x % 10 == 1) {
                    rs.push("&nbsp;&nbsp;");
                ...
                rs.push("<font style='font-size:0.35em;line-height:5px;color:black;font-family:SimHei;'>" + tmp.Name + "</font>");
                if (x % 16 == 0) {
                    rs.push("&nbsp;&nbsp;");
                } else if (x % 16 == 1) {
                    rs.push("&nbsp;&nbsp;");
