...
function CreateCross(tablename, true_data__false_url, obj, request, easyuiparse) {
...
function CreateTable(tablename, rowCount, cellCount) {
    var table = "<table id='X"+tablename+"'  cellpadding='1' cellspacing='1' border='1'  class='easyui-datagrid' > ";//
    table += "<thead>";
    table += "<tr>";
    var i = 0;
    for (var j = 0; j < cellCount; j++) {
        if (debug) {
            table += "<th id='" + tablename + "_" + i + "_" + j + "' field='H" + i + "_" + j + "' width='150'>" + i + ":" + j + "</th>";
        } else {
            table += "<th id='" + tablename + "_" + i + "_" + j + "'  field='H" + i + "_" + j + "' width='150'></th>";
        }
    }
    table += "</tr>";
    table += "</thead>";
...
function onccSelect(record) {
    ...
    var top = 10;
    //alert(value);
    CreateCross("dg2", false, "../api/Cross/GetCrossProjectNodeFiles", { "Project": project,"Top":10 }, true);
}
